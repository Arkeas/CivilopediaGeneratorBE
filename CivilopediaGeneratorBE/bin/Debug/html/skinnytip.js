...
function makeHTML(text, blnimage, title) {
	...
	if (!blnimage) {
		container_style += "background-image: url(\'/civilopediabe/images/tipcentre.png\');";
		container_style += "line-height: 27px;";
	}
	else {
		container_style += "background-image: url(\'/civilopediabe/images/small/PROMOTION_BLANK.png\'); height: 64px; width: 64px;";
	}
	...
	if (!blnimage)
		text = "<img src=\'/civilopediabe/images/tipleft.png\' style=\'height: 27px; width: 6px;\' />" + text + "<img src=\'/civilopediabe/images/tipright.png\' style=\'height: 27px; width: 6px;\' />";
...
function cboLanguage_SelectedIndexChanged() {
	var objItem = document.getElementById("cboLanguage");
	var objSelectedItem = objItem.options[objItem.selectedIndex];
	var strLanguage = objSelectedItem.value;
	if (strLanguage != "")
		strLanguage += '/';
	var strPath = window.location.pathname;
	var strPage = strPath.substring(strPath.lastIndexOf('/') + 1);
	var strURL = "/civilopediabe/" + strLanguage + strPage;
